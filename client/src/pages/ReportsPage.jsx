  ...
  const renderBody = () => {
      ...
      return (
        <>
          <div className="flex gap-4 mb-4">
            {[['Issued', s.issued], ['Returned', s.returned], ['Active', s.active], ['Overdue', s.overdue]].map(([l, v]) => (
                  ...
                  <tbody>
                    {members.map((m) => (
                      <tr key={m._id} style={{ borderBottom: '1px solid #f8f8f8' }}>
                        ...
                        <td className="py-2 px-4 text-xs">{m.role}</td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl border p-4" style={{ backgroundColor: '#fff', borderColor: '#f0f0f0' }}>
            ...
            {popular.length === 0 ? (
              ...
              <div key={p.book?.bookId} className="flex items-center justify-between py-2" style={{ borderBottom: '1px solid #f0f0f0' }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold" style={{ color: '#2C2C3E' }}>{p.book?.title}</p>
                    <p className="text-xs" style={{ color: '#94a3b8' }}>{p.book?.author}</p>
                  </div>
                </div>
                <span className="text-sm font-bold" style={{ color: '#1a1245' }}>{p.count} borrows</span>
              </div>
            ))}
          </div>
          <div className="rounded-xl border p-4" style={{ backgroundColor: '#fff', borderColor: '#f0f0f0' }}>
    ...
    if (reportType === 'fines') {
      ...
      return (
        <>
          <div className="flex gap-4 mb-4">
            {[['Collected', fSummary.totalCollected || 0], ['Outstanding', fSummary.totalOutstanding || 0], ['Waived', fSummary.totalWaived || 0], ['Records', fSummary.count || 0]].map(([l, v]) => (
              <div key={l} className="px-4 py-3 rounded-xl flex-1" style={{ backgroundColor: '#fff', border: '1px solid #e0e0e0' }}>
                <p className="text-xs font-medium" style={{ color: '#94a3b8' }}>{l}</p>
                <p className="text-xl font-bold" style={{ color: '#1a1245' }}>{typeof v === 'number' && v > 100 ? `&#8360; ${v.toFixed(2)}` : v}</p>
              </div>
            ))}
          </div>
          <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: '#fff', borderColor: '#f0f0f0' }}>
